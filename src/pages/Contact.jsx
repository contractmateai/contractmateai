...
const Contact = () => {
  ...
  useEffect(() => {
    // AOS and particles are loaded globally via index.html
    if (window.AOS) window.AOS.init({ once: true, duration: 800 });
    ...
    if (container) {
      ...
      function sideBiasedPercent() {
        const r = Math.random();
        if (r < 0.375) { return Math.random() * 20; }
        else if (r < 0.75) { return 80 + Math.random() * 20; }
        else { return 20 + Math.random() * 60; }
      }
      for (let i = 0; i < NUM; i++) {
